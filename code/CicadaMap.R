...
# Adding a new column to for circle color
Circles$color <- ifelse(Circles$Circle %in% c("Circle 1", "Circle 8"), "black", "red")

# Eno River map 
ggmap(Eno.map) +
  geom_point(data = Circles, aes(x = longitude, y = latitude, color = color), 
             pch = 19, size = 4, face = "bold", show.legend = FALSE) +
