...
snakes <- read.csv("copperheads/data/snakes/inat-serpentes.csv")

#load in cicada and county data
cicada <- st_read(dsn = "copperheads/data/cicada/periodical_cicada_with_county.gdb")

#turn the snake lat/lon into a point eg. make geometry
...
check <- st_contains(cicada, snakes_geom)
#hm, okay, I think that gave me all the snake rows associated with each polygon.

#let's try this way:
snakes_county <- 
  st_join(snakes_geom, cicada, join = st_within) %>%
  filter(!is.na(STATEFP))
#cool, so we have 178,000 snake observations within the area. 
...
n_copperhead <-
  snakes_county %>%
  filter(species == "Agkistrodon contortrix") %>%
  group_by(ST_CNTY_CODE, species) %>%
...
n_copperhead_year <- 
  snakes_county %>%
  filter(species == "Agkistrodon contortrix") %>%
  group_by(ST_CNTY_CODE, species, year) %>%
  summarize(n_copperhead = n()) %>%
  st_drop_geometry() %>%
  ungroup()
...
n_copperhead_brood_year <-
  snakes_brood %>%
  filter(species == "Agkistrodon contortrix") %>%
  group_by(BROOD_NAME, year, species) %>%
